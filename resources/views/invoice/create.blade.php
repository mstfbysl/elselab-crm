...
@section('title', __('locale.Create New Invoice'))
...
@section('content')
<input type="hidden" id="create-invoice-total">
...
<section class="invoice-add-wrapper">
    <div class="row invoice-add">
            ...
            <div class="card invoice-preview-card">
                ...
                <div class="card-body invoice-padding pb-0">
                    <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0">
                        <div>
                            <h3 class="text-primary invoice-logo">{{ __('locale.Create New Invoice') }}</h3>
                            <hr>
                        ...
                        <div class="invoice-number-date mt-md-0 mt-2">
                            <div class="d-flex align-items-center justify-content-md-end mb-1">
                                ...
                                <div class="input-group input-group-merge">
                                    <input type="text" class="form-control" id="create-invoice-no" placeholder="" disabled/>
                                </div>
                            </div>
                            <div class="d-flex align-items-center mb-1">
                                <span class="title">{{ __('locale.Date') }}:</span>
                                <input type="text" class="form-control date-picker" id="create-invoice-date"/>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row row-bill-to invoice-spacing">
                        <div class="col-xl-8 mb-lg-1 col-bill-to ps-0">
                            ...
                            <div class="invoice-customer">
                                ...
                                <hr>
                                <select class="form-select" id="create-invoice-client-id">
                                    <option></option>
                                </select>
                ...
                <div class="card-body invoice-padding invoice-product-details">
                    <div class="create-invoice-item-list">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="mb-2">
                                ...
                                <textarea class="form-control" rows="2" id="note" placeholder="Invoice Note" id="create-invoice-note"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-xl-3 col-md-4 col-12">
            ...
            <div class="mt-2">
                <p class="mb-50">{{ __('locale.Serie') }}</p>
                <select class="form-select" id="create-invoice-serie">
                    <option value=""></option>
                </select>
            </div>
            <div class="mt-2">
                <p class="mb-50">{{ __('locale.Type') }}</p>
                <select class="form-select" id="create-invoice-type">
                    <option value=""></option>
                </select>
            </div>
            <div class="mt-2">
                <p class="mb-50">{{ __('pdf.Issued by') }}</p>
                <select class="form-select" id="create-invoice-user-id">
                    <option value=""></option>
                </select>
            </div>
        </div>
...
$(async function() {
    ...
    FillSelect2('#create-invoice-type', '/api/system-definition/list-filter', false, [1], {type: 'invoice_type'});  
    
    // window.newSerie = async () => {
    //     let new_invoice_no = await AxiosGET('/api/invoice/new-invoice-no');
    //     $('#create-invoice-no').val(new_invoice_no.data.data);
    // }
    let serie = $('#create-invoice-serie').val();

    $('#create-invoice-serie').on('select2:select', function (e) {
        let serie = $(this).val();
        let date = $('#create-invoice-date').val();
        update_serie(serie, date);
    });


    /* Client Changed */
    $('#create-invoice-client-id').change(() => {

        let currentSelected = $('#create-invoice-client-id').val();
    ...
    /* Invoice Item List */
    var invoiceItemList = $('.create-invoice-item-list');
    ...
    window.onCreateInvoice = async () => {

        let client_id = $('#create-invoice-client-id').val();
        let user_id = $('#create-invoice-user-id').val();
        let invoice_no = $('#create-invoice-no').val();
        let invoice_serie = $('#create-invoice-serie').val();
        let type = $('#create-invoice-type').val();
        let date = $('#create-invoice-date').val();
        let note = $('#create-invoice-note').val();

        if(!invoice_serie){
            ToastAlert(window.JSLang['Serie must be selected!'], 'warning');
            return;
        }
        ...
        let check_invoice_serie = await AxiosPOST('/api/invoice/check-invoice-serie', {invoice_no});

        if(check_invoice_serie.data !== null){
            ToastAlert(['Invoice number changed.'], 'info');
            update_serie($('#create-invoice-serie').val(), $('#create-invoice-date').val());
            invoice_no = $('#create-invoice-no').val();
        }
    ...
    function update_serie(serie, date){
        AxiosPOST('/api/invoice/new-invoice-no', {serie, date}, (r) => {
            let response = r.data;
            if(response.status == true){
                $('#create-invoice-no').val(response.data);
            }
        });
    }
