...
<input type="hidden" id="create-offer-total">
...
$(async function() {
    ...
    var $repeater = offerItemList.repeater({
        show: function() {
            ...
            $('.create-offercreate-offer-item-list input').keyup(itemCalculator);
    ...
    AxiosGET('/api/offer/details-from-project/' + id, (r)  => {

        let res = r.data;

        if(res.status == true){

            $('#create-offer-title').val(res.data.title);

            FillSelect2('#create-offer-client-id', '/api/client/list-all', false, [res.data.client_id]); 
            FillSelect2('#create-offer-user-id', '/api/user/list-all', false, [res.data.owner_id]); 
